...
class TestMQTTClientIdUsage:
    ...
    @patch("malla.mqtt_capture.time.sleep", side_effect=KeyboardInterrupt)
    def test_main_uses_configured_client_id(
        ...
        call_args, call_kwargs = mock_mqtt_client_class.call_args
        assert call_args == (CallbackAPIVersion.VERSION2,)
        assert call_kwargs["client_id"] == "test-client-id"
        assert call_kwargs["userdata"]["name"] == "default"
